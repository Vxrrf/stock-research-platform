...
def refresh_freshness(rec, cfg):
    """Recompute data_freshness_status + confidence from last_updated vs NOW.
    Must be called when reprocessing cached records (e.g. --from-cache), otherwise
    a week-old quote keeps its original 'FRESH' label (Codex review fix)."""
    return _freshness_confidence(rec, cfg)
